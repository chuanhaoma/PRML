...
def k_fold_val_DenseNet_A(
    ...
    for fold in range(k):
        ...
        # 进行训练
        obj = save_obj(model, optimizer, scheduler, loss_fn, DEVICE, batch_size, random_state) # 打包参数
        obj, history, best_acc = train(obj, train_dataloader, test_dataloader, epochs, False, save_model, f"dense_old{fold}") # 训练
...
if True: # 0.9885+-0.0049
    EPOCHS = 75
    best_accs, mean, std, histories = k_fold_val_DenseNet_A(epochs=EPOCHS, save_model=True)
    save_report('./model/dense_report.txt', best_accs, mean, std, EPOCHS)
