...
from model import ViTForPollenClassification, DenseNet201ForPollenClassification, HybridModelForPollenClassification
...
def k_fold_val_Hybrid_A(
    ...
    for fold in range(k):
        ...
        # 模型
        model = HybridModelForPollenClassification(all_weight=True).to(DEVICE)
...
if True:
    EPOCHS = 75
    best_accs, mean, std, histories = k_fold_val_Hybrid_A(epochs=EPOCHS)
    ...
    for i in range(len(histories)):
        save_figure(histories[i], f"./figure/{i:02d}.png", f"Fine-tuning on Hybrid Model - Fold {i} Epoch {len(histories[i]['train_loss'])}")
